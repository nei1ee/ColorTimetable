...
<template>
  <div class="overflow-y-scroll" :style="{ height: `calc(100vh - ${appStore.customBarHeight}px)` }">
    ...
    <div class="min-h-max pb-safe grid grid-flow-col p-1 z-20 gap-1 grid-rows-10 grid-cols-[0.7fr_repeat(7,1fr)]">
      ...
      <template
        v-for="(weekCourse, weekIndex) of courseStore.semesterCourseList[courseStore.currentWeekIndex]"
        :key="weekIndex"
      >
        <template v-for="(dayCourse, dayIndex) of weekCourse" :key="dayIndex">
          <div
            v-if="dayCourse.length"
            class="border-white rounded-lg border-2 border-opacity-50 text-center p-1 transition-background-color col-span-1 duration-300 relative box-content "
